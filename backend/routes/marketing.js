...
router.get('/', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await (async () => {
      ...
      const countR = await pool.query('SELECT COUNT(*) FROM marketing_campaigns');
      const total = parseInt(countR.rows[0].count);
      const r = await pool.query('SELECT * FROM marketing_campaigns ORDER BY created_at DESC LIMIT $1 OFFSET $2', [limit, offset]);
