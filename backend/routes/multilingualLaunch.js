/*
 * Multilingual launch optimizer: Adapts metadata and marketing for 10+ regions
 *
 * Custom Feature Suggestion (apply pass 6 — audit batch_01)
 * Suggestion: Adapts metadata and marketing for 10+ regions
 *
...
router.post('/run', async (req, res) => {
  try {
    if (!process.env.OPENROUTER_API_KEY) {
      return res.status(503).json({ error: 'AI unavailable', missing: 'OPENROUTER_API_KEY', feature: 'multilingual_launch' });
    }
    const payload = req.body || {};
    const userPrompt = `Context (JSON):\n${JSON.stringify(payload).slice(0, 4000)}\n\nTask: ${'Multilingual launch optimizer'}\n\nReturn JSON {"summary": string, "actions": [string], "risks": [string], "confidence": number_0_to_1}.`;
    const systemPrompt = `You are a $'multilingual book-launch optimizer'. Respond with strict JSON.`;
    const aiResp = await callOpenRouter(userPrompt, systemPrompt);
    const content = aiResp?.choices?.[0]?.message?.content ?? aiResp?.raw ?? '';
    const parsed = parseAIJson(content);
    return res.json({ feature: 'multilingual_launch', model: aiResp?.model, parsed, raw: content });
  } catch (err) {
...
router.get('/health', (req, res) => res.json({ feature: 'multilingual_launch', ok: true, ai_configured: !!process.env.OPENROUTER_API_KEY }));
