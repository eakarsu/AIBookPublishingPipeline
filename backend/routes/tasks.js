...
router.get('/', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await (async () => {
      ...
      const countR = await pool.query('SELECT COUNT(*) FROM publishing_tasks');
      const total = parseInt(countR.rows[0].count);
      const r = await pool.query('SELECT * FROM publishing_tasks ORDER BY due_date ASC, priority DESC LIMIT $1 OFFSET $2', [limit, offset]);
