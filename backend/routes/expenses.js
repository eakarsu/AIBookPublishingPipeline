...
router.get('/', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await (async () => {
      ...
      const countR = await pool.query('SELECT COUNT(*) FROM expenses');
      const total = parseInt(countR.rows[0].count);
      const r = await pool.query('SELECT * FROM expenses ORDER BY expense_date DESC LIMIT $1 OFFSET $2', [limit, offset]);
